...
JavaScript 是弱类型脚本语言，变量的数据类型是解释时动态决定的，基本数据类型有如下 5 个：
...
JavaScript 中的对象本质上是一个关联数组，由 `key - value` 对组成。特别地，JavaScript 对象的 value 不仅可以是值，也可以是方法。
...
> 可以这样理解：增加了 `prototype` 属性的类继承了原有的类，这就是 JavaScript 所提供的「伪继承机制」。
