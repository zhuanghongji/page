# 进一步了解 JavaScript 中的 Number、Array、Date 和 Boolean
...
可以使用小数点也可以不使用小数点来表示数字。

所有的数字都存储为「根为 10 的 64 位 (8 比特)」的浮点数。

整数的精度最多为 15 位，消失的为 17 位，浮点运算不总是 100% 正确。
...
例如，`radix` 为 2，该数字就会被转换为二进制值表示的字符串。
...
console.log(i.toFixed(1))   // 12.3
console.log(j.toFixed(1))   // 12.4
...
* `num`: 必需。规定指数计数法中的小数位数，是 0 ~ 20 之间的值。如果省略了该参数，将使用尽可能多的数字。
...
> 不使用 new 运算符时，行为与使用 new 运算符调用它时的行为完全一样。
...
> `splice()` 方法可删除从 index 处开始的零个或多个元素，并且用参数列表中声明的零个或多个值来替换那些被删除的元素。如果从 arrayObject 中删除了元素，则返回的是含有被删除元素的新数组。
...
`Boolean` 对象可将「非逻辑值」转换为「逻辑值 ( `true` / `false`)」。
